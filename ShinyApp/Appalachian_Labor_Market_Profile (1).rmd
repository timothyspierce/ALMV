...
```{r Add_RUCC}
appal2 <- readRDS("appal2.RDS")
d <- appal2 %>%
  group_by(nonmetro.f) %>%
  summarise_at(vars(age0_14, age15_64, age65plus, LT_HS, HS_Dip, SomeColl,Coll_Plus, OwnHome, Pct.HI, Pct.Unemp, Pct.Dis, PerCapInc, tt014,tt1529,tt3044,tt4559,tt60plus, HH.Pct.Broadband), funs(mean(., na.rm=TRUE)))
...
readRenviron("~/.Renviron") #readin Census API key
#Create a nice list of FIPS state and county. Use `fip_list` below to pull variables and to map.
counties<-read.csv("data/All_Mining.csv", header=T)
#Note Yellowstone National Park did not have ACS data so drop it.
counties <- filter(counties,FIPS!= 30113)
#only keep Appalachia
counties <- filter(counties, coal_region== 1)
...
```{r Add_RUCC}
urb_ru <- read.csv("data/ruralurbancodes2013.csv", header=T)

#add in a leading zero and make it into a character format 
urb_ru$GEOID <- sprintf("%05d",urb_ru$FIPS)
...
saveRDS(poly_all_ACS5, file = "data/poly_all_ACS5.Rds")
...
>>>>>>> 3620ee7703eb3fd86a5300460aaba2d886da7530
